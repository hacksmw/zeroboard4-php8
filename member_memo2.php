...
if(intval($_zbDefaultSetup['memo_limit_time'])!==0) 
	zb_query("delete from $send_memo_table where member_no='$member[no]' and (".time()." - reg_date) >= ".$_zbDefaultSetup['memo_limit_time']) or error(zb_error());
...
if ($exec=="del_all") {
	...
	movepage("$PHP_SELF?page=".urlencode($page));
}
...
if($page>$show_page_num) {
	...
	$print_page='<a href="'.$PHP_SELF.'?id='.urlencode($id).'&page='.urlencode($prev_page).'&select_arrange='.urlencode($select_arrange).'&desc='.urlencode($desc).'&category='.urlencode($category).'&sn='.urlencode($sn).'&ss='.urlencode($ss).'&sc='.urlencode($sc).'&keyword='.urlencode($keyword).'&page_num='.urlencode($page_num).'">[Prev]</a> ';
	$print_page.='<a href="'.$PHP_SELF.'?id='.urlencode($id).'&page=1&select_arrange='.urlencode($select_arrange).'&desc='.urlencode($desc).'&category='.urlencode($category).'&sn='.urlencode($sn).'&ss='.urlencode($ss).'&sc='.urlencode($sc).'&keyword='.urlencode($keyword).'&page_num='.urlencode($page_num).'">[1]</a>..';
}
...
<!-- 선택된 메모가 있을때;; -->
<?php
 if(isset($now_data) && isset($now_data['no'])) {
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="3">
        <tr>
          ...
          <td><img src="images/t.gif" width="10" height="3"><br>
            <a href=javascript:void(window.open('view_info.php?member_no=<?=urlencode($now_data['member_to'])?>','view_info','width=400,height=510,toolbar=no,scrollbars=yes'))><?=htmlspecialchars($now_data['name'])?></a> <font style=font-size:8pt;>(<b>ID</b> : <?=htmlspecialchars($now_data['user_id'])?>)</td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td><a href="<?=$PHP_SELF?>?exec=del&no=<?=urlencode($no)?>&page=<?=urlencode($page)?>" onclick="return confirm('삭제하시겠습니까?')"><img src="images/memo_delete2.gif" width="31" height="15" border=0></a> <a href="<?=$PHP_SELF?>"><img src="images/memo_list.gif" width="18" height="15" border=0></a> </td>
        </tr>
      </table>
    </td>
...
?>
        ...
        <tr onMouseOver=this.style.backgroundColor="#FFF5F5" onMouseOut=this.style.backgroundColor="">
          ...
          <td style='word-break:break-all;' style=cursor:hand; onclick=location.href="<?="$PHP_SELF?exec=view&no=".urlencode($data['no'])."&page=".urlencode($page).""?>"><img src="images/t.gif" width="10" height="3"><br>
            <a href=<?="$PHP_SELF?exec=view&no=".urlencode($data['no'])."&page=$page"?>><?=$data['subject']?></a></td>
          <td width="80" align="center"><img src="images/t.gif" width="10" height="3"><br>
            <a href=javascript:void(window.open('view_info.php?member_no=<?=urlencode($data['member_to'])?>','view_info','width=400,height=510,toolbar=no,scrollbars=yes'))><?=$data['name']?></a><br><font style=font-size:8pt;color:999999>(<?=htmlspecialchars($data['user_id'])?>)</td>
