...
$mode = req('mode');
if ($mode) {
	if (!($mode == "write" || $mode == "reply" || $mode == "modify")) {
		error("mode가 올바르지 않습니다.");
	}
}
...
elseif($setup['grant_write']<$member['level']&&!$is_admin) 
	Error("사용권한이 없습니다","login.php?id=".urlencode($id)."&page=".urlencode($page)."&page_num=".urlencode($page_num)."&category=".urlencode($category)."&sn=".urlencode($sn)."&ss=".urlencode($ss)."&sc=".urlencode($sc)."&keyword=".urlencode($keyword)."&no=".urlencode($no)."&file=zboard.php");
if($mode=="reply"&&$setup['grant_view']<$member['level']&&!$is_admin) 
	Error("사용권한이 없습니다","login.php?id=".urlencode($id)."&page=".urlencode($page)."&page_num=".urlencode.($page_num)."&category=".urlencode($category)."&sn=".urlencode($sn)."&ss=".urlencode($ss)."&sc=".urlencode($sc)."&keyword=".urlencode($keyword)."&no=".urlencode($no)."&file=zboard.php");
...
if($mode=="modify"&&$data['ismember']) {
	if($data['ismember']!=$member['no']&&!$is_admin&&$member['level']>$setup['grant_delete']) Error("사용권한이 없습니다","login.php?id=".urlencode($id)."&page=".urlencode($page)."&page_num=".urlencode($page_num)."&category=".urlencode($category)."&sn=".urlencode($sn)."&ss=".urlencode($ss)."&sc=".urlencode($sc)."&keyword=".urlencode($keyword)."&no=".urlencode($no)."&file=zboard.php");
}
...
if($setup['use_category']) {
	...
	while($category_data=mysql_fetch_array($category_result)) {
		if($data['category']==$category_data['no']) 
			$category_kind.="<option value=$category_data[no] selected>$category_data[name]</option>";
		else 
			$category_kind.="<option value=$category_data[no]>$category_data[name]</option>";
	}
...
if ($mode=="modify") {
	...
	if($data['is_secret']&&!$is_admin&&$data['ismember']!=$member['no']&&$_SESSION['zb_s_check']!=$setup['no']."_".$no) 
		error("정상적인 방법으로 수정하세요");

	$name=isset($data['name']) ? ($data['name']) : ''; // 이름
	$email=isset($data['email']) ? ($data['email']) : ''; // 메일
	$homepage=isset($data['homepage']) ? ($data['homepage']) : ''; // 홈페이지 
	$subject=$data['subject']=isset($data['subject']) ? ($data['subject']) : ''; // 제목
//	$sitelink1=isset($sitelink1) ? str_replace("\"","&quot;",$sitelink1) : '';
//	$sitelink2=isset($sitelink2) ? str_replace("\"","&quot;",$sitelink2) : '';
	$memo=$data['memo']; // 내용
	$sitelink1=$data["sitelink1"]=isset($data['sitelink1']) ? ($data["sitelink1"]) : '';
	$sitelink2=$data["sitelink2"]=isset($data['sitelink2']) ? ($data["sitelink2"]) : '';
	
	if($data['file_name1'])$file_name1="<br>&nbsp;".htmlspecialchars($data["s_file_name1"])."이 등록되어 있습니다. <input type=checkbox name=del_file1 value=1> 삭제";
	if($data['file_name2'])$file_name2="<br>&nbsp;".htmlspecialchars($data["s_file_name2"])."이 등록되어 있습니다. <input type=checkbox name=del_file2 value=1> 삭제";

	if($data['use_html']) $use_html=" checked ";
	if($data['reply_mail']) $reply_mail=" checked "; else $reply_mail = '';
	if($data['is_secret']) $secret=" checked "; else $secret = '';
	if($data['headnum']<=-2000000000) $notice=" checked "; else $notice = '';
...
// 공지기능 사용하는지 않하는지 표시;;
if((!$is_admin&&$member['level']>$setup['grant_notice'])||$mode=="reply") { $hide_notice_start="<!--";$hide_notice_end="-->"; } else { $hide_notice_start="";$hide_notice_end=""; }
...
head(" onload=unlock() onunload=hideImageBox() ");

include_once "script/script_write.php";
...
if(!($notice)) $notice = '';
if(!($memo)) $memo = '';
if(!($sitelink1)) $sitelink1 = '';
if(!($sitelink2)) $sitelink2 = '';
if(!($file_name1)) $file_name1 = '';
if(!($file_name2)) $file_name2 = '';
if(!($category_kind)) $category_kind = '';
if(!($use_html)) $use_html = '';
if(!($reply_mail)) $reply_mail = '';
if(!($secret)) $secret = '';
if(!($upload_limit)) $upload_limit = '';
