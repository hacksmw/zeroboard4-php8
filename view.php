...
// 처음 한번만 실행
if (!defined("_view_first_run")) {
	define("_view_first_run", true);
	...
	unset($next_data);
	unset($next_name);
	unset($hide_next_start);
	unset($hide_next_end);
	unset($next_data);
	unset($next_comment_num);
	unset($a_next);
	unset($next_subject);
	unset($next_hit);
	unset($next_vote);
	...
	unset($a_next_email);
	unset($next_icon);
	unset($next_face_image);
	unset($next_mail);
	
	unset($prev_email);
	unset($prev_data);
	unset($prev_name);
	unset($hide_prev_start);
	unset($hide_prev_end);
	unset($prev_data);
	unset($prev_comment_num);
	unset($a_prev);
	unset($prev_subject);
	unset($prev_hit);
	unset($prev_vote);
	...
	unset($a_prev_email);
	unset($prev_icon);
	unset($prev_face_image);
	unset($prev_mail);
	
	unset($hide_c_password_start);
	unset($hide_c_password_end);
...
if($setup['grant_view']<$member['level']&&!$is_admin) 
	Error("사용권한이 없습니다","login.php?id=".urlencode($id)."&page=".urlencode($page)."&page_num=".urlencode($page_num)."&category=".urlencode($category)."&sn=".urlencode($sn)."&ss=".urlencode($ss)."&sc=".urlencode($sc)."&keyword=".urlencode($keyword)."&no=".urlencode($no)."&s_url=".urlencode($REQUEST_URI));

if (!$no || !is_numeric($no)) {
	error("게시물 번호를 지정해야 합니다.");
}
...
if($data['is_secret']&&!$is_admin&&$data['ismember']!=$member['no']&&$member['level']>$setup['grant_view_secret']) {
	if($member['no']) {
		...
		$secret_check=mysql_fetch_array(zb_query("select count(*) from $t_board"."_$id where headnum='$data[headnum]' and password='".get_password($password, false ,true)."'"));
		if(!$secret_check[0]&&strlen(get_password("a"))>=41) $secret_check=mysql_fetch_array(zb_query("select count(*) from $t_board"."_$id where headnum='$data[headnum]' and password='".get_password($password, true, true)."'"));
		if(!$secret_check[0]) {
			head();
			$a_list="<a onfocus=blur() href='zboard.php?".htmlspecialchars($href).htmlspecialchars($sort)."'>";    
			$a_view="<Zeroboard ";
...
if($data['prev_no']&&!$setup['use_alllist']) {
	...
	$prev_subject=$prev_data['subject']=htmlspecialchars($prev_data['subject'])." ".$prev_comment_num;
	...
	if(!isBlank($prev_email)||$prev_data['ismember']) {
		if(!$setup['use_formmail']) $a_prev_email="<a onfocus=blur() href='mailto:".htmlspecialchars($prev_email)."'>";
		else $a_prev_email="<a onfocus=blur() href=\"javascript:void(window.open('view_info.php?to=".urlencode($prev_email)."&id=".urlencode($id)."&member_no=$prev_data[ismember]','mailform','width=400,height=500,statusbar=no,scrollbars=yes,toolbar=no'))\">";
...
if($data['next_no']&&!$setup['use_alllist']) {
	...
	$next_subject=$next_data['subject']=htmlspecialchars($next_data['subject'])." ".$next_comment_num;
	...
	if(!isBlank($next_email)||$next_data['ismember']) {
		if(!$setup['use_formmail']) $a_next_email="<a onfocus=blur() href='mailto:".htmlspecialchars($next_email)."'>";
		else $a_next_email="<a onfocus=blur() href=\"javascript:void(window.open('view_info.php?to=".urlencode($next_email)."&id=".urlencode($id)."&member_no=$next_data[ismember]','mailform','width=400,height=500,statusbar=noscrollbars=yes,toolbar=no'))\">";
...
if($data['homepage']) $a_homepage="<a onfocus=blur() href='".htmlspecialchars($data["homepage"])."' target=_blank>"; else $a_homepage="<Zetx"; // 홈페이지 주소 링크
...
if(!isBlank($email)||$data['ismember']) {
	if(!$setup['use_formmail']) $a_email="<a onfocus=blur() href='mailto:".htmlspecialchars($email)."'>";
		else $a_email="<a onfocus=blur() href=\"javascript:void(window.open('view_info.php?to=".urlencode($email)."&id=$id&member_no=$data[ismember]','mailform','width=400,height=500,statusbar=no,scrollbars=yes,toolbar=no'))\">";
...
// 목록 버튼
if($is_admin||$member['level']<=$setup['grant_list']) $a_list="<a onfocus=blur() href='zboard.php?id=".urlencode($id)."&page=".urlencode($page)."&page_num=".urlencode($page_num)."&category=".urlencode($category)."&sn=".urlencode($sn)."&ss=".urlencode($ss)."&sc=".urlencode($sc)."&keyword=".urlencode($keyword)."&prev_no=".urlencode($no)."&sn1=".urlencode($sn1)."&divpage=".urlencode($divpage)."&select_arrange=".urlencode($select_arrange)."&desc=".urlencode($desc)."'>"; else $a_list="<Zeroboard  ";
...
if($member['no']) {
	$c_name=htmlspecialchars($member['name']); $hide_c_password_start="<!--"; $hide_c_password_end="-->"; 
	...
	$c_name="<input type=text name=name size=8 maxlength=10 class=input value=\"".htmlspecialchars($_SESSION['zb_writer_name'])."\">";
}
...
if($setup['use_comment']) {
	while($c_data=mysql_fetch_array($view_comment_result)) {
		$comment_name=htmlspecialchars($c_data['name']);
