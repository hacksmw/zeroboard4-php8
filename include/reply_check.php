...
$memo=$reply_data['memo']=nl2br(xss2($reply_data['memo'])); // 내용
$memo=autolink($memo); // 자동링크 거는 부분;;
$hit=$reply_data['hit'];  // 조회수
$vote=$reply_data['vote'];  // 투표수
if($setup['use_showip']||$is_admin)$ip="IP Address : ".htmlspecialchars($reply_data['ip'])."&nbsp;";  // 아이피
...
if(eregi("\.jpg",$file_name1)||eregi("\.gif",$file_name1)||eregi("\.png",$file_name1)) $upload_image1="<img src='".urlencode($reply_data['file_name1'])."' border=0><br>";
if(eregi("\.jpg",$file_name2)||eregi("\.gif",$file_name2)||eregi("\.png",$file_name2)) $upload_image2="<img src='".urlencode($reply_data['file_name2'])."' border=0><br>";
...
if(!isBlank($email)||$reply_data['ismember']) {
	if(!$setup['use_formmail']) $name="<a href='mailto:".htmlspecialchars($email)."'>".htmlspecialchars($name)."</a>";
	else $name="<a href=javascript:void(window.open('view_info.php?to=".urlencode($email)."&id=".urlencode($id)."&member_no=".urlencode($reply_data['ismember'])."','mailform','width=400,height=500,statusbar=no,scrollbars=yes,toolbar=no'))>".htmlspecialchars($name)."</a>";
}
...
if($no==$reply_data['no']) $number="<img src=$dir/arrow.gif border=0 align=absmiddle>"; elseif($number!="&nbsp;") $number=htmlspecialchars($roop_number);
