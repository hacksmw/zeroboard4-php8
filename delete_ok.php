...
if (!eregi("delete.php",$HTTP_REFERER)) Error("제대로 된 접근을 하여 주시기 바랍니다");
...
if (isset($member['is_admin']) && $member['is_admin'] == 1 || isset($member['is_admin']) && $member['is_admin'] == 2 && 
	$member['group_no'] == $setup['group_no'] || check_board_master($member, $setup['no'])) {
	$is_admin=1;
} else {
	$is_admin="";
}
...
if ($password) {
	...
	if (strlen($s_data['password']) <= 16 && strlen(get_password("a")) >= 41) {
		$password = get_password(req("password"), true, true);
	}
}
...
movepage("zboard.php?id=".urlencode($id)."&page=".urlencode($page)."&page_num=".urlencode($page_num)."&select_arrange=".urlencode($select_arrange)."&desc=".urlencode($des)."&sn=".urlencode($sn)."&ss=".urlencode($ss)."&sc=".urlencode($sc)."&keyword=".urlencode($keyword)."&sn1=".urlencode($sn1)."&divpage=".urlencode($divpage));
