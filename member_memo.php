...
// 지정 넘은 글 삭제;;
if(intval($_zbDefaultSetup['memo_limit_time']) !== 0) 
	zb_query("delete from $get_memo_table where member_no='$member[no]' and (".time()." - reg_date) >= ".$_zbDefaultSetup['memo_limit_time']) or error(zb_error());
...
if($page>$show_page_num) {
	...
	$print_page="<a href='$PHP_SELF?id=".urlencode($id)."&page=".urlencode($prev_page)."&select_arrange=".urlencode($select_arrange)."&desc=".urlencode($desc)."&category=".urlencode($category)."&sn=".urlencode($sn).
	"&ss=".urlencode($ss)."&sc=".urlencode($sc)."&keyword=".urlencode($keyword)."&page_num=".urlencode($page_num)."'>[Prev]</a> ";
	$print_page.="<a href='$PHP_SELF?id=".urlencode($id)."&page=1&select_arrange=".urlencode($select_arrange)."&desc=".urlencode($desc)."&category=".urlencode($category)."&sn=".urlencode($sn)."&ss=".urlencode($ss)."&sc=".
		urlencode($sc)."&keyword=".urlencode($keyword)."&page_num=".urlencode($page_num)."'>[1]</a>..";
}

while($i+$start_page<=$total_page&&$i<=$show_page_num) {
	$move_page=$i+$start_page;
	if($page==$move_page) $print_page.=" <b>$move_page</b> ";
	else $print_page.="<a href='$PHP_SELF?id=".urlencode($id)."&page=".urlencode($move_page)."&select_arrange=".urlencode($select_arrange)."&desc=".urlencode($desc)."&category=".urlencode($category)."&sn=".urlencode($sn)."&ss=".urlencode($ss)."&sc=".urlencode($sc)."&keyword=".urlencode($keyword)."&page_num=".urlencode($page_num)."'>[$move_page]</a>";
	$i++;
}

if($total_page>$move_page) {
	$next_page=$move_page+1;
	$print_page.="..<a href='$PHP_SELF?id=".urlencode($id)."&page=".urlencode($total_page)."&select_arrange=".urlencode($select_arrange)."&desc=".urlencode($desc)."&category=".urlencode($category)."&sn=".urlencode($sn)."&ss=".urlencode($ss)."&sc=".urlencode($sc)."&keyword=".urlencode($keyword)."&page_num=".urlencode($page_num)."'>[$total_page]</a>";
	$print_page.=" <a href='$PHP_SELF?id=".urlencode($id)."&page=".urlencode($next_page)."&select_arrange=".urlencode($select_arrange)."&desc=".urlencode($desc)."&category=".urlencode($category)."&sn=".urlencode($sn)."&ss=".urlencode($ss)."&sc=".urlencode($sc)."&keyword=".urlencode($keyword)."&page_num=".urlencode($page_num)."'>[Next]</a>";
}
...
<table width="100%" border="0" cellspacing="0" cellpadding="5">
  <tr>
    ...
    <td align=right><img src="images/t.gif" width="10" height="10"><font color="#666666">전체 :
      <b><?=htmlspecialchars($total)?></b> , 새 쪽지 : <b><?=htmlspecialchars($new_total)?></b></font>&nbsp;&nbsp;&nbsp;&nbsp;</td>
  </tr>
</table>
...
<?php
	if(isset($now_data['no'])) {
		...
		if($temp_name) $now_data['name']="<img src='".htmlspecialchars($temp_name)."' border=0 align=absmiddle>";
		$temp_name = get_private_icon($now_data['member_from'], "1");
		if($temp_name) $now_data['name']="<img src='".htmlspecialchars($temp_name)."' border=0 align=absmiddle>&nbsp;".htmlspecialchars($now_data['name']);
?>
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="3">
        <tr> 
          ...
          <td><img src="images/t.gif" width="10" height="3"><br>
            <a href="javascript:void(window.open('view_info.php?member_no=<?=urlencode($now_data['member_from'])?>','view_info','width=400,height=500,toolbar=no,scrollbars=yes'))"><?=htmlspecialchars($now_data['name'])?></a> <font style=font-size:8pt;>(<b>ID</b> : <?=htmlspecialchars($now_data['user_id'])?>)</td>
        </tr>
        <tr> 
        ...
        <tr> 
          ...
          <td style='word-break:break-all;'><img src="images/t.gif" width="10" height="3"><br>
            <?=autolink(nl2br((del_html($now_data['memo']))))?><br>
            <br>
          </td>
        </tr>
        <tr>
          <td align="right" valign="top">&nbsp;</td>
          <td><a href=javascript:void(window.open('view_info.php?member_no=<?=urlencode($now_data['member_from'])?>','view_info','width=400,height=500,toolbar=no,scrollbars=yes'))><img src="images/memo_reply.gif" width="28" height="15" border=0></a> <a href=<?=$PHP_SELF?>?exec=del&no=<?=urlencode($no)?>&page=<?=urlencode($page)?> onclick="return confirm('삭제하시겠습니까?')"><img src="images/memo_delete2.gif" width="31" height="15" border=0></a> <a href=<?=$PHP_SELF?>><img src="images/memo_list.gif" width="18" height="15" border=0></a> </td>
        </tr>
      </table>
    </td>
...
<?php
	...
	while($data=mysql_fetch_array($result)) {
		...
		if($temp_name) $data['name']="<img src='".htmlspecialchars($temp_name)."' border=0 align=absmiddle>";
		$temp_name = get_private_icon($data['member_from'], "1");
		if($temp_name) $data['name']="<img src='$temp_name' border=0 align=absmiddle>&nbsp;".htmlspecialchars($data['name']);
...
?>
        ...
        <tr onMouseOver=this.style.backgroundColor="#FFF5F5" onMouseOut=this.style.backgroundColor=""> 
          <td width="20" align="center" height="23"> 
            <input type=checkbox name="del[]" value="<?=htmlspecialchars($data['no'])?>">
          </td>
          ...
          <td style='word-break:break-all;' style=cursor:hand; onclick=location.href="<?="$PHP_SELF?exec=view&no=$data[no]&page=$page"?>"><img src="images/t.gif" width="10" height="3"><br>
            <a href=<?="$PHP_SELF?exec=view&no=$data[no]&page=$page"?>><?=htmlspecialchars($data['subject'])?></a></td>
          <td width="80" align="center"><img src="images/t.gif" width="10" height="3"><br>
            <a href=javascript:void(window.open('view_info.php?member_no=<?=urlencode($data['member_from'])?>','view_info','width=400,height=510,toolbar=no,scrollbars=yes'))><?=htmlspecialchars($data['name'])?></a><br><font style=font-size:8pt;color:999999>(<?=htmlspecialchars($data['user_id'])?>)</td>
          <td width="60" align="center"><font style=font-family:Tahoma;font-size:8pt;><span title='<?=htmlspecialchars($reg_date)?>'><?php echo"".date("m/d",$data['reg_date'])."" ?></span></font></td>
        </tr>
<?php
 		$loop_number--;
	}
