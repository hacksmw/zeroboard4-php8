...
$zx = $_REQUEST["zx"];
$zy = $_REQUEST["zy"];
$reply_mail = $_REQUEST["reply_mail"];
$is_secret = $_REQUEST["is_secret"];
$del_file1 = $_REQUEST["del_file1"];
$del_file2 = $_REQUEST["del_file2"];
$file_name1 = $_REQUEST["file_name1"];
...
if ($use_html && !is_numeric($use_html)) {
	error("번호가 올바르지 않습니다.");
}
...
if(!$is_admin) {
	$memo = isset($_REQUEST["use_html"]) ? zb_sxs(zb_script_conv($memo)) : $memo;
}

// 관리자이거나 HTML허용레벨이 낮을때 태그의 금지유무를 체크
if(!$is_admin&&$setup['grant_html']<$member['level']) {
	// 내용의 HTML 금지;;
	if((!isset($_REQUEST["use_html"])) || $setup['use_html'] == 0) $memo=del_html($memo);

	// HTML의 부분허용일때;;
	if(isset(($_REQUEST["use_html"])) && $setup['use_html']==1) {
		$memo = zero_script_conv($setup['avoid_tag'],$memo);
	}
} else {
	if(!isset(($_REQUEST["use_html"]))) {
		$memo=del_html($memo);
	}
}
...
if(($is_admin||$member['level']<=$setup['use_html'])&&isset($_REQUEST["use_html"])) $subject=addslashes($subject);
else $subject=addslashes(($subject));
	
$memo=addslashes($memo);

$reply_mail = addslashes($reply_mail);
$is_secret = addslashes($is_secret);
	
if(isset($_REQUEST["use_html"]) && $use_html < 2) {
	$memo=str_replace("  ","&nbsp;&nbsp;",$memo);
	$memo=str_replace("\t","&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;",$memo);
}	

$sitelink1 = $_REQUEST["sitelink1"];
$sitelink2 = $_REQUEST["sitelink2"];

$sitelink1=($sitelink1) ? addslashes(del_html($sitelink1)) : '';
$sitelink2=($sitelink2) ? addslashes(del_html($sitelink2)) : '';
$email=($email) ? addslashes(del_html($email)) : '';
...
if($mode=="modify"&&$no) {
	...
	if($s_data['ismember']!=$member['no']&&!$is_admin) {
		$_REQUEST['password']=isset($_REQUEST['password']) ? zb_escape_string($_REQUEST['password']) : '';
		if(strlen($s_data['password'])<=16&&strlen(get_password("a"))>=41) $password=get_password($_REQUEST["password"], true);
	...
	// 일반글 -> 공지 
	else if (!empty($notice)&&$s_data['headnum']>-2000000000) {
		$temp=mysql_fetch_array(zb_query("select max(division) from $t_division"."_$id"));
		$max_division=$temp[0];
		$temp=mysql_fetch_array(zb_query("select max(division) from $t_division"."_$id where num>0 and division!='$max_division'"));
		if(!$temp[0]) $second_division=0; else $second_division=$temp[0];
 ...
 **************************************************************************/
	...
	$file_name1 = addslashes($file_name1);
	$file_name2 = addslashes($file_name2);
	$s_file_name1 = addslashes($s_file_name1);
	$s_file_name2 = addslashes($s_file_name2);
...
movepage($view_file."?id=$id&page=$page&page_num=".urlencode($page_num)."&select_arrange=".urlencode($select_arrange)."&desc=".urlencode($des)."&sn=".urlencode($sn)."&ss=".urlencode($ss)."&sc=".urlencode($sc)."&keyword=".urlencode($keyword)."&no=".urlencode($no)."&category=".urlencode($category)."");
