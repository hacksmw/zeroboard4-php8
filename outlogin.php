...
<?php
 ...
 function print_bbs($skinname, $title, $id, $num=5, $textlen=30, $datetype="Y/m/d") {
	...
	if(eregi("\[notice\_",$header)) {
		...
		if(eregi("\.gif|\.jpg",$filename1))$uploadimage1 = "<img src=".$_zb_url.htmlspecialchars($filename1)." border=0><br>"; else $uploadimage1="";
		if(eregi("\.gif|\.jpg",$filename2))$uploadimage2 = "<img src=".$_zb_url.htmlspecialchars($filename1)." border=0><br>"; else $uploadimage2="";
		...
		$subject = htmlspecialchars(cut_str(stripslashes($data['subject']),$textlen))."</font></b>";
	...
	while($data=mysql_fetch_array($result)) {
		...
		$subject = htmlspecialchars(cut_str(($data['subject']),$textlen))."</font></b>";
		$date = date($datetype, $data['reg_date']);
 ...
 function print_survey($skinname, $title, $id, $textlen=30) {
	...
	$main_subject="<a href='".$_zb_url.$target."&no=$no'>".htmlspecialchars($tmpData['subject'])."</a>";
	...
	while($data=mysql_fetch_array($result)) {
		...
		$main = $loop;
		$main = str_replace("[subject]","<a href='".$_zb_url."apply_vote.php?id=$id&no=$no&sub_no=$data[no]'>".htmlspecialchars($subject)."</a>",$main);
...
function print_gallery($skinname, $title, $id, $num=10, $xsize=80, $ysize=80, $xnum=10) {
	...
	$tmpResult = zb_query("select use_alllist from $admin_table where name='$id'") or die(zb_error());
	$setup = mysql_fetch_array($tmpResult);
	if($setup['use_alllist']) $target = "zboard.php?id=".urlencode($id);
	else $target = "view.php?id=".urlencode($id);
	...
	while($data=mysql_fetch_array($result)) {
		...
		if($filename) $imgList.="<a href='".$_zb_url.$target."&no=$data[no]'><img src='".htmlspecialchars($filename)."' border=1 style=border-color:black width=$xsize height=$ysize vspacing=10 hspacing=10></a>";
